...
\lstinputlisting[style=cstyle,caption=Простейший DAYTIME клиент,label=lst:date-client-basic]{src/network/date-client-basic.c}

Ниже приведен пример запуска клиента для сервера \texttt{time.ien.it}.

\begin{plainlst}{Использование}{lst:basic-client-run}
...
\lstinputlisting[style=cstyle,caption=DAYTIME клиент,label=lst:date-client]{src/network/date-client.c}
...
\lstinputlisting[style=cstyle,caption=DAYTIME сервер,label=lst:date-server]{src/network/date-server.c}
